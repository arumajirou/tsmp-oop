...
def upsert_meta(engine, meta_table, idx_table, futr_cols, hist_cols, stat_cols):
    with engine.begin() as con:
        ...
        con.execute(
            text(f"INSERT INTO {meta_table}(id, futr_exog_list, hist_exog_list, stat_exog_list) VALUES (1, :f::text[], :h::text[], :s::text[]"),
...
def build_features_pandas(df: pd.DataFrame, max_lag: int) -> pd.DataFrame:
    ...
    grng["stat_start_ord"] = grng["min"].astype("int64")
    grng["stat_end_ord"]   = grng["max"].astype("int64")
...
stat_only = stat.drop(columns=["unique_id"], errors="ignore").reset_index(drop=True)
